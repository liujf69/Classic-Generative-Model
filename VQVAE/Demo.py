...
class Process():
    def __init__(self, batch_size = 256, num_training_iteration = 1500, num_hiddens = 128, num_residual_layers = 2, num_residual_hiddens = 32,
    ...
    def load_model(self):
        self.model = Model(self.num_hiddens, self.num_residual_layers, self.num_residual_hiddens,
                    self.num_embeddings, self.embedding_dim, self.commitment_cost, self.decay).to(self.device)
    ...
    def train(self):
        ...
        for i in range(self.num_training_iteration):
            data, _ = next(iter(self.training_loader)) # data, label
...
def main():
    ...
    mytest = Process(batch_size = batch_size, 
                    num_training_iteration = num_training_iteration, 
