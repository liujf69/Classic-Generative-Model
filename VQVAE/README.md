# VQVAE
# 推荐论文和博客
1. [VQVAE-PyTorch-实现教程](https://zhuanlan.zhihu.com/p/640000410)
2. [Neural Discrete Representation Learning](https://arxiv.org/pdf/1711.00937)

# 网络结构
```
VQ-VAE三个核心组成模块：编码器（Encoder）、解码器（Decoder）和向量量化器（VectorQuantizer）。
```
## Encoder
```python
# Encoder主要由Conv层和Residual层组成。
...
## Decoder
```python
# Decoder主要由Conv层, Residual层和ConvTranspose2d层组成。
...
## VectorQuantizer
```python
## VQ的核心思想是：计算Encoder输出与码本Codebook中Embedding的距离，选择最相似的Embedding作为Decoder的输入。
## 建议结合论文（https://arxiv.org/pdf/1711.00937）中的公式来阅读代码
